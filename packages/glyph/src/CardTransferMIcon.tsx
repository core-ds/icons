/**
 * This is auto-generated file by scripts/generate.ts
 * Do not edit it manually
 */
import React, { SVGProps } from 'react';

export const CardTransferMIcon: React.FC<SVGProps<SVGSVGElement>> = props => (
    <svg role="img" focusable="false" fill="currentColor" width="24" height="24" viewBox="0 0 24 24" {...props}><path d="M2 4.5A2.5 2.5 0 014.5 2h10A2.5 2.5 0 0117 4.5V9H8.8C6.701 9 5 10.79 5 13h-.5A2.5 2.5 0 012 10.5v-6z"/><path d="M22 13v1H7v-1a2 2 0 012-2h11a2 2 0 012 2z"/><path d="M9 22a2 2 0 01-2-2v-4h15v4a2 2 0 01-2 2H9zm0-4h3v1.5H9V18z" fillRule="evenodd" clipRule="evenodd"/></svg>
);

export default CardTransferMIcon;
