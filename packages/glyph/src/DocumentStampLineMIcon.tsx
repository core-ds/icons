/**
 * This is auto-generated file by scripts/generate.ts
 * Do not edit it manually
 */
import React, { SVGProps } from 'react';

export const DocumentStampLineMIcon: React.FC<SVGProps<
    SVGSVGElement
>> = props => (
    <svg
        role="img"
        focusable="false"
        fill="currentColor"
        width="24"
        height="24"
        viewBox="0 0 24 24"
        {...props}
    >
        <path d="M16 15.5a2.5 2.5 0 11-5 0 2.5 2.5 0 015 0z" />
        <path
            d="M13.813 2.669C13.21 2.216 12.447 2 11.568 2H4v20h16V10.815c0-.981-.208-1.818-.645-2.486-.442-.678-1.064-1.1-1.737-1.337-.792-.279-1.675-.31-2.514-.225.09-.8.076-1.633-.145-2.383-.189-.642-.542-1.262-1.146-1.715zM6 20V4h5.568c.551 0 .862.132 1.045.269.183.137.33.35.427.68.209.71.12 1.744-.151 2.802a1 1 0 001.204 1.22c1.1-.267 2.146-.344 2.86-.092.327.114.563.29.727.542.171.262.32.69.32 1.394V20H6z"
            fillRule="evenodd"
            clipRule="evenodd"
        />
    </svg>
);

export default DocumentStampLineMIcon;
