/**
 * This is auto-generated file by scripts/generate.ts
 * Do not edit it manually
 */
import React, { SVGProps } from 'react';

export const ChainCircleMMIcon: React.FC<SVGProps<SVGSVGElement>> = props => (
    <svg
        role="img"
        focusable="false"
        width="24"
        height="24"
        viewBox="0 0 24 24"
        {...props}
        fill="none"
    >
        <path
            d="M12 22c5.523 0 10-4.477 10-10S17.523 2 12 2 2 6.477 2 12s4.477 10 10 10zm4.628-10.309l-.859.859-1.099-1.1.858-.858a1.424 1.424 0 000-2.014l-.106-.106a1.424 1.424 0 00-2.014 0l-1.717 1.716a1.425 1.425 0 000 2.014l.107.107c.123.123.262.218.411.287l-1.132 1.132a3.001 3.001 0 01-.378-.32l-.107-.107a2.979 2.979 0 010-4.212l1.717-1.717a2.979 2.979 0 014.212 0l.107.107a2.979 2.979 0 010 4.212zm-9.256.617l.859-.858 1.099 1.1-.858.858a1.424 1.424 0 000 2.014l.106.106a1.424 1.424 0 002.014 0l1.717-1.716a1.425 1.425 0 000-2.014l-.107-.107a1.423 1.423 0 00-.411-.287l1.132-1.132c.133.094.26.201.378.32l.107.107a2.979 2.979 0 010 4.212l-1.717 1.717a2.979 2.979 0 01-4.212 0l-.107-.107a2.979 2.979 0 010-4.213z"
            fill="#000"
            fillRule="evenodd"
            clipRule="evenodd"
        />
    </svg>
);

export default ChainCircleMMIcon;
